...
export const qualityOrder: GraphicsQuality[] = ["performance", "low", "medium", "high", "gpu"];
export type EffectsQuality = {
  smoke: boolean; sparks: boolean; speedParticles: boolean;
  maxSmoke: number; maxSparks: number; maxSpeedParticles: number; maxSkids: number;
  sceneryStep: number; minimalDrawCalls: boolean;
};
type Preset = {
  dpr: number | [number, number]; antialias: boolean; shadows: boolean;
  importedPlayer: boolean; enhancedEnvironment: boolean; anisotropy: number;
  effects: EffectsQuality;
  environment: {
    fogNear: number; fogFar: number; skyRadius: number;
    sun: [number, number, number]; sunIntensity: number; hemisphere: number;
    shadowSize: number; shadowExtent: number;
    mountainDetail: number; slicePlants: number; plantCutoff: number;
  };
  forest: { candidates: number; near: number; mid: number; far: number; shadow: number };
  ground: { candidates: number; distance: number };
};
const highEffects: EffectsQuality = { smoke: true, sparks: true, speedParticles: true, maxSmoke: 70, maxSparks: 36, maxSpeedParticles: 36, maxSkids: 115, sceneryStep: 6, minimalDrawCalls: false };
const high: Preset = {
...
export const qualityPresets: Record<GraphicsQuality, Preset> = {
