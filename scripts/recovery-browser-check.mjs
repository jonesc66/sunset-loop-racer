...
const out=path.resolve('verification/recovery-20260912/runtime');
...
try{
  ...
  if(['manual','controls'].includes(mode)){
   ...
   }else if(mode==='art-recovery'){
    for(const preset of ['high','gpu']){
      await quality(preset);
      for(const zone of ['A','B','C','D','E','F','G','H','I','J','I-entrance','I-exit'])await view(zone,preset);
    }
    for(const preset of ['perf','high','gpu','high']){await quality(preset);report.checks.push({switch:preset,scene:await sceneState()});}
    ...
    if(mode==='regression'){
      const saved=await evaluate("JSON.parse(localStorage.getItem('bern-circuit-time-leaderboard-v2-expanded-1525m-3laps'))");
      if(saved?.race.length!==1||saved?.lap.length!==1||saved.race[0].playerName==='Legacy'||saved.race[0].time<=55.3)throw Error('New-track records missing or contaminated');
      const ui=await state();if(ui.includes('Legacy'))throw Error('Legacy record shown');
      report.checks.push({expandedRecords:saved,legacyExcluded:true});
    }
