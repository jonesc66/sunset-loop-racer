...
const out='verification/multitrack-phase1/';
...
try{
 ...
 const stopped=await evaluate(sensor);await keys(['W']);await delay(6000);await keys([]);const fast=await evaluate(sensor);assert(fast.speed>stopped.speed+5);assert(fast.distance<15);assert.notDeepEqual(fast.position,stopped.position);assert.notDeepEqual(fast.ai,initial.ai);report.checks.accelerationAIAndCamera='PASS';report.fast=fast;await shot('driving-w');
 await keys(['A']);await delay(2500);await keys([]);const left=await evaluate(sensor);await keys(['D']);await delay(2500);await keys([]);const right=await evaluate(sensor);assert(Math.abs(left.heading-fast.heading)>.001);assert(Math.abs(right.heading-left.heading)>.001);report.checks.steeringAD='PASS';
 await keys(['S']);await delay(2500);await keys([]);const braking=await evaluate(sensor);assert(braking.speed<right.speed);report.checks.brakeS='PASS';
 await keys(['Space']);await delay(500);await keys([]);await keys(['R']);await wait('('+sensor+').speed<1',30000);await keys([]);const reset=await evaluate(sensor);assert(reset.speed<1);report.checks.resetR='PASS';report.reset=reset;
 ...
 while(Date.now()-begin<900000){
  ...
  if(Date.now()-lastLog>5000){report.trace.push(s);console.log('DRIVE',s.lap,s.cp,'progress',s.progress.toFixed(3),'speed',s.speed.toFixed(1));lastLog=Date.now();await fs.writeFile(out+'runtime-progress.json',JSON.stringify(report,null,2));}
