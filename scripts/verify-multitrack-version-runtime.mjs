...
const out='verification/multitrack-phase1/version-fix-runtime/';
...
async function click(label){const p=await evaluate(`(()=>{const b=[...document.querySelectorAll('button')].find(b=>b.textContent.trim().toLowerCase()===${JSON.stringify(label.toLowerCase())});if(!b)throw Error('Missing button');b.scrollIntoView({block:'center'});const r=b.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()`);if(label.toLowerCase()==='race again'){await delay(500);await shot('result-bottom');}
 await cmd('Input.dispatchMouseEvent',{type:'mousePressed',button:'left',clickCount:1,...p});await cmd('Input.dispatchMouseEvent',{type:'mouseReleased',button:'left',clickCount:1,...p});}
...
const report={controlMode:'Existing DEV autoRace for follow-up persistence and high-speed camera validation; first run uses actual player keyboard driving',started:new Date().toISOString(),checks:{},trace:[],errors,network};
...
try{
 ...
 await cmd('Page.navigate',{url:base+'?autoRace'});await wait("document.querySelector('.playerNameField input')");
 report.storageBefore=await evaluate('({...localStorage})');assert(JSON.parse(report.storageBefore[storageKey]??'null')?.race?.length>0,'First race must already be saved before follow-up');report.checks.load='PASS';
 ...
 await cmd('Emulation.setDeviceMetricsOverride',{width:1280,height:960,deviceScaleFactor:1,mobile:false});await delay(2000);
 await keys([]);assert(report.finish,'Keyboard-driven three-lap race did not finish');report.checks.autoRaceThreeLaps='PASS';assert(maxSpeed>40,'Follow-up run must exercise higher-speed camera');assert(maxDistance<15,'Camera must not fall behind');report.checks.highSpeedCamera='PASS';report.maxSpeed=maxSpeed;report.maxDistance=maxDistance;
 ...
 const saved=JSON.parse(report.storageAfter[storageKey]);
 const completedPlayer=(await evaluate('window.__raceTestState')).results.find(r=>r.isPlayer);
 assert(saved.race.some(r=>Math.abs(r.time-completedPlayer.time)<1e-8),'This race must be written to the original key');
 assert(saved.lap.some(r=>Math.abs(r.time-completedPlayer.bestLapTime)<1e-8),'This lap must be written to the original key');
 report.checks.currentResultMatchesStoredRecord='PASS';assert(saved.race.length>0&&saved.lap.length>0);assert(report.resultText.includes('Personal Top 5'));report.checks.resultAndSave='PASS';
