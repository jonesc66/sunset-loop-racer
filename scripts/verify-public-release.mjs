...
const out=path.resolve('verification/pages-release');
...
const base='https://jonesc66.github.io/sunset-loop-racer/';
...
const failedResponses=[];ws.addEventListener('message',e=>{const m=JSON.parse(e.data);if(m.method==='Network.responseReceived'&&m.params.response.status>=400)failedResponses.push({url:m.params.response.url,status:m.params.response.status});});
try{
 await navigate(base);await waitFor("document.querySelectorAll('[data-map-car]').length===5",90000);
 const initial=await evaluate("({player:document.querySelector('[data-map-player]')?.getAttribute('transform'),readonly:document.querySelector('[data-race-controls]')?.readOnly})");if(!initial.player||!initial.readonly)throw Error('Latest minimap/keyboard changes not present');
 await click('gpu');await waitFor("document.querySelector('.activeQuality')!==null && [...document.querySelectorAll('.activeQuality')].some(b=>b.textContent.toLowerCase()==='gpu')");
 await waitFor("performance.getEntriesByType('resource').some(r=>r.name.includes('forest-v1.glb'))",120000);await delay(8000);
