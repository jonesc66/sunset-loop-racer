...
type QualityConfig = {
  smoke: boolean;
  sparks: boolean;
  speedParticles: boolean;
  maxSmoke: number;
  maxSparks: number;
  maxSpeedParticles: number;
  maxSkids: number;
  sceneryStep: number;
  minimalDrawCalls: boolean;
};
...
const QUALITY_CONFIG: Record<GraphicsQuality, QualityConfig> = {
  performance: {
    smoke: false,
    sparks: false,
...
function TrackSurface({ track }: { track: TrackInfo }) {
  const roadGeometry = useMemo(() => createRoadGeometry(track), [track]);
  const dashGeometry = useMemo(() => createDashGeometry(track), [track]);
  const { gl } = useThree();
  const asphalt = useMemo(() => makeRoadMaterial(Math.min(8, gl.capabilities.getMaxAnisotropy())), [gl]);
...
export default function RaceScene({
  ...
  const qualityConfig = QUALITY_CONFIG[graphicsQuality];
        ...
        >
          {graphicsQuality === "performance" ? (
            <PerformanceCarModel
              ...
              color={car.color}
            />
          ) : graphicsQuality === "high" && car.isPlayer ? (
