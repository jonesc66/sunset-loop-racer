...
function centerAt(progress: number) {
...
export function forwardDelta(from: number, to: number) {
  return wrapProgress(to - from);
}

export function crossedProgress(last: number, current: number, target: number) {
  if (current >= last) {
...
function makeSample(progress: number): TrackSample {
...
export function createTrack(): TrackInfo {
  const samples = Array.from({ length: TRACK_SAMPLE_COUNT }, (_, index) =>
    makeSample(index / TRACK_SAMPLE_COUNT)
  );
  ...
  const checkpointTargets = Array.from(
    { length: CHECKPOINT_COUNT - 1 },
    (_, index) => (index + 1) / CHECKPOINT_COUNT
  );
...
export function sampleTrack(track: TrackInfo, progress: number) {
...
export function nearestTrackSample(track: TrackInfo, position: THREE.Vector3) {
  let bestIndex = 0;
...
export function tangentHeading(tangent: THREE.Vector3) {
  return Math.atan2(tangent.x, tangent.z);
}

export function createRoadGeometry(track: TrackInfo) {
  ...
  for (let index = 0; index <= track.samples.length; index++) {
    ...
    if (index > 0) distance += sample.center.distanceTo(track.samples[index - 1].center);
...
export function createDashGeometry(track: TrackInfo) {
  const vertices: number[] = [];
  const indices: number[] = [];
